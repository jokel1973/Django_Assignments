...
def create_account(request):
    form = AccountForm(request.POST or None) # Retrieve the Account form
    # Checks if request method is POST
    if request.method == 'POST':
...
def balance(request, pk):
    ...
    for t in transactions: # Loop through transactions and determine which is a deposit or withdrawal
        if t.type == 'Deposit':
            ...
            table_contents.update({t: current_total}) # If deposit add amount to balance
        else:
...
def transaction(request):
    form = TransactionForm(request.POST or None) # Retrieves the Transaction form
    # Checks if request method is POST
    if request.method == 'POST':
        if form.is_valid(): # Check to see if the submitted form is valid and if so, saves the form
